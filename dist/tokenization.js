...
exports.unigramsAndBigrams = unigramsAndBigrams;
const { log } = Math;
const tiny = 1e-10;
function l(k, n, x) {
...
function score(countBigram, count1, count2, nWords) {
    if (nWords <= count1 || nWords <= count2)
        return 0;
    const N = nWords;
...
function processTokens(words, normalizePlurals = true) {
    ...
    for (const [lower, perCase] of caseCounts.entries()) {
        ...
        const representative = mostCommon !== null && mostCommon !== void 0 ? mostCommon : Array.from(perCase.keys())[0];
...
function unigramsAndBigrams(words, stopwords, normalizePlurals = true, collocationThreshold = 30) {
    ...
    const { counts: countsBigrams } = processTokens(bigrams.map((b) => b.join(' ')), normalizePlurals);
