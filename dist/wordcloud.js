...
exports.ColormapColorFunc = exports.WordCloud = void 0;
exports.defaultColorFunc = defaultColorFunc;
...
function ensureFont(fontPath, family) {
    if (!fontPath || !family)
        return;
    if (canvas_1.GlobalFonts.has(family))
        return;
    if (fs_1.default.existsSync(fontPath)) {
        canvas_1.GlobalFonts.registerFromPath(fontPath, family);
    }
}
function defaultColorFunc(_word, _fontSize, _position, _orientation, random) {
    const r = random || new random_1.default();
...
class ColormapColorFunc {
    ...
    __call__(_word, _fontSize, _position, _orientation, random, _meta) {
        return this.call(random);
    }
}
exports.ColormapColorFunc = ColormapColorFunc;
class IntegralOccupancyMap {
    ...
    samplePosition(sizeX, sizeY, rng) {
        ...
        if (sizeX > height || sizeY > width)
            return null;
        let hits = 0;
        for (let i = 0; i < height - sizeX; i++) {
            for (let j = 0; j < width - sizeY; j++) {
                const area = integral[i * width + j] +
...
class WordCloud {
    constructor(options = {}) {
        ...
        this.preferHorizontal = (_a = options.preferHorizontal) !== null && _a !== void 0 ? _a : 0.9;
    ...
    processText(text) {
        ...
        if (this.collocations) {
            counts = (0, tokenization_1.unigramsAndBigrams)(words, stopwords, this.normalizePlurals, this.collocationThreshold);
        }
        else {
            const filtered = words.filter((w) => !stopwords.has(w.toLowerCase()));
            counts = (0, tokenization_1.processTokens)(filtered, this.normalizePlurals).counts;
        }
        return counts;
    }
    ...
    generateFromFrequencies(frequencies, maxFontSize, bootstrap = false) {
        ...
        let lastFreq = 1;
        let fontSize;
        const entries = this.repeat && normalized.length < this.maxWords
        ...
        for (const [word, freq] of entries) {
            ...
            occupancy.occupyPixels(drawY, drawX, imgData.data, tmpW, tmpH);
            layoutItems.push({
                word,
                freq,
                fontSize,
    ...
    padFrequencies(frequencies) {
        if (!this.repeat || frequencies.length === 0)
            return frequencies;
